...
G1–G4 方法论受 **Matt Pocock 的生产级 Skill 体系**启发并汉化落地（学习来源：`mattpocock/grill-me` · `to-spec` · `to-tickets` · `implement`，2026-07 汉化适配为企业场景，四道闸门逻辑逐条对应原 Skill 设计）。G0 开局上下文装配、KPI 台账整合与记忆护照打通为**本项目的独立增量**。感谢原作者的工程实践。

---

## 一句话

多个 AI Agent（Claude / Codex / Trae / GPT 等）在同一台机器上干活，没人管就互相翻文件、写脏数据、烧 token。**Agent Gates** 是一套开工纪律：每次干活前先过闸门（G0 上下文装配 → G1 盘问 → G2 规格 → G3 工单 → G4 验收），跳门自动扣分，不需要人盯着。
